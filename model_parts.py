...
def mian_att_unit(querys, keys):
    keys_length = tf.shape(keys)[1] #
    embedding_size = querys.get_shape().as_list()[-1]
    keys = tf.reshape(keys, shape=[-1, keys_length, embedding_size])#batch * n * embedding
    querys = tf.reshape(tf.tile(querys, [1, keys_length, 1]), shape=[-1, keys_length, embedding_size])
    net = tf.concat([keys, querys], axis=-1)
    att_wgt = tf.layers.dense(net, units=1, activation=tf.tanh)        #batch * n * 1
    scores = tf.reshape(att_wgt, shape=[-1, 1, keys_length], name="weight")  #batch * 1 * n
    scores = tf.nn.softmax(scores)
    outputs = tf.matmul(scores, keys)    #(batch_size, 1, embedding_size)
    outputs = tf.reduce_sum(outputs, 1, name="attention_embedding")   #(batch_size, embedding_size)
    return outputs
